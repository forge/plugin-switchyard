...
/**
 * Command to promote a component-level service to a composite-level service.
 */
public class PromoteServiceCommand extends AbstractSwitchYardCommand {
    ...
    {
       return Metadata.from(super.getMetadata(context), getClass()).name("SwitchYard: Promote Service")
                .description("Promote a SwitchYard Service")
    ...
    public Result execute(UIExecutionContext shellContext) throws Exception {
        ...
        switchYardConfigurator.promoteService(project, serviceName.getValue());
        	
        shellContext.getUIContext().getProvider().getOutput().out().println(serviceName.getValue() + " promoted.");
